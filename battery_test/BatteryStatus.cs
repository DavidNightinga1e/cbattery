...
namespace cbattery
{
    ...
    {
        public static BatteryInformation GetBatteryInfo()
        {
            return BatteryInfo.GetBatteryInformation();
        }

        public static string GetStringInfo()
        {
            // TODO User to choose parameters in tray
            var batteryInfo = BatteryInfo.GetBatteryInformation();
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.Append("C: ");
            stringBuilder.Append(((float)batteryInfo.CurrentCapacity / 1000).ToString("0.0"));
            stringBuilder.Append("/");
            stringBuilder.Append(((float)batteryInfo.FullChargeCapacity/ 1000).ToString("0.0"));
            stringBuilder.Append(" Wh");
            stringBuilder.AppendLine();
            stringBuilder.AppendLine();
            
            stringBuilder.Append(((float)batteryInfo.Voltage / 1000).ToString("0.0"));
            stringBuilder.Append("V");
            stringBuilder.AppendLine();

            stringBuilder.Append("Wear ");
            stringBuilder.Append(((float)(batteryInfo.DesignedMaxCapacity - batteryInfo.FullChargeCapacity) * 100 / batteryInfo.DesignedMaxCapacity).ToString("0.0"));
            stringBuilder.Append("%");
            stringBuilder.AppendLine();
            ...
            if (batteryInfo.DischargeRate != 0)
            {
                ...
                stringBuilder.Append(((float)batteryInfo.DischargeRate / 1000).ToString("0.0"));
                stringBuilder.Append(" Wh");
                stringBuilder.AppendLine();
                stringBuilder.Append("T ");
                var time = (float) batteryInfo.CurrentCapacity * (-1) / batteryInfo.DischargeRate;
                var hours = time.ToString("0");
                var min = ((time - Math.Floor(time)) * 60).ToString("00");
                stringBuilder.Append($"{hours}:{min}");
            }
            else
            {
